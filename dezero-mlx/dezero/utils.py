"""
DeZero Utilities Module
-----------------------
그래프 시각화, gradient check, 다운로드 등 유틸리티 함수
"""
import os
import subprocess
...
# =============================================================================
# Utility functions for numpy / mlx
# =============================================================================
def sum_to(x, shape):
    """Sum elements along axes to output an array of a given shape."""
    xp = MLX.get_array_module(x)
    ndim = len(shape)
    lead = x.ndim - ndim
...
def logsumexp(x, axis=1):
    """Stable log-sum-exp for NumPy/MLX."""
    xp = MLX.get_array_module(x)
    m = xp.max(x, axis=axis, keepdims=True)
    y = x - m
    y = xp.exp(y)
    s = xp.sum(y, axis=axis, keepdims=True)
    s = xp.log(s)
    return m + s
...
def gradient_check(f, x, *args, rtol=1e-4, atol=1e-5, **kwargs):
    """Automatic backward checker via finite differences."""
    x = as_variable(x)
    x.data = MLX.as_numpy(x.data).astype(np.float64)

    num_grad = numerical_grad(f, x, *args, **kwargs)
    y = f(x, *args, **kwargs)
    y.backward()
    bp_grad = MLX.as_numpy(x.grad.data)
...
def numerical_grad(f, x, *args, **kwargs):
    ...
    x_var = x if isinstance(x, Variable) else None
    x_arr = x.data if isinstance(x, Variable) else x
    xp = MLX.get_array_module(x_arr)

    x_np = MLX.as_numpy(x_arr).astype(np.float64, copy=True)
    ...
    while not it.finished:
        ...
        if xp is np:
            ...
            to_backend = getattr(MLX, 'asarray', None)
            x_pos = to_backend(x_pos_np) if to_backend else x_pos_np
            x_neg = to_backend(x_neg_np) if to_backend else x_neg_np

        def call_f(arr):
            if x_var is not None:
                x_tmp = Variable(arr)
                return f(x_tmp, *args, **kwargs)
            else:
        ...
        y1_np = MLX.as_numpy(y1)
        y2_np = MLX.as_numpy(y2)

        diff = (y1_np - y2_np).sum()
        grad_np[idx] = diff / (2 * eps)
    ...
    if xp is np:
        return grad_np
    to_backend = getattr(MLX, 'asarray', None)
    return to_backend(grad_np) if to_backend else grad_np
...
def array_equal(a, b):
    ...
    a, b = MLX.as_numpy(a), MLX.as_numpy(b)
    return np.array_equal(a, b)


def array_allclose(a, b, rtol=1e-4, atol=1e-5):
    """Returns True if two arrays(or variables) are element-wise close."""
    a = a.data if isinstance(a, Variable) else a
    b = b.data if isinstance(b, Variable) else b
    a, b = MLX.as_numpy(a), MLX.as_numpy(b)
