"""
DeZero Layers Module
--------------------
Linear, Conv2d, RNN, LSTM 등 레이어 정의
"""
import os
import weakref
import numpy as np
import dezero.functions as F
from dezero import cuda as MLX  # ★ 수정: cuda를 MLX로 import
from dezero.core import Parameter
from dezero.utils import pair
from dezero.core import Parameter, Variable  # Variable 추가
...
class EmbedID(Layer):
    def __init__(self, in_size, out_size):
        super().__init__()
        self.W = Parameter(np.random.randn(in_size, out_size).astype(np.float32), name='W')

    def __call__(self, x):
        # x가 Variable이면 data 추출
        if isinstance(x, Variable):
            x_data = x.data
        else:
            x_data = x
        
        # embed_id 함수 사용 (gradient 지원)
        return F.embed_id(x_data, self.W)
